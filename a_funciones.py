...
def img2data(path, width=100):
    ...
    list_labels = [path+f for f in listdir(path)] ### crea una lista de los archivos en la ruta (fractured / not fractured)
    ...
    for imagePath in list_labels: ### recorre cada carpeta de la ruta ingresada
        ...
        for item in tqdm(files_list): ### le pone contador a la lista: tqdm
            ...
            if file[-1] =='g': ### verificar que la imágen sea extensión jpg o jpeg
                ...
                if l == 'not fractured':  ### verificar en qué carpeta está para asignar el label
                    labels.append([0])
                elif l == 'fractured':
                    labels.append([1])
